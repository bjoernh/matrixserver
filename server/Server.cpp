...
void Server::setupDispatcher() {
    ...
    dispatcher_.registerHandler(matrixserver::registerApp,
        std::bind(&Server::handleRegisterApp, this, _1, _2));
    dispatcher_.registerHandler(matrixserver::getServerInfo,
        std::bind(&Server::handleGetServerInfo, this, _1, _2));
    dispatcher_.registerHandler(matrixserver::requestScreenAccess,
        std::bind(&Server::handleRequestScreenAccess, this, _1, _2));
    dispatcher_.registerHandler(matrixserver::setScreenFrame,
        std::bind(&Server::handleSetScreenFrame, this, _1, _2));
    ...
    dispatcher_.registerHandler(matrixserver::appAlive,
        std::bind(&Server::handleAppLifecycle, this, _1, _2));
    ...
    dispatcher_.registerHandler(matrixserver::appKill,
        std::bind(&Server::handleAppLifecycle, this, _1, _2));
    dispatcher_.registerHandler(matrixserver::appParamSchema,
        std::bind(&Server::handleAppParamSchema, this, _1, _2));
    dispatcher_.registerHandler(matrixserver::appParamValues,
        std::bind(&Server::handleAppParamValues, this, _1, _2));
}
...
void Server::handleGetServerInfo(
    std::shared_ptr<UniversalConnection> conn,
    std::shared_ptr<matrixserver::MatrixServerMessage> /*msg*/) {
    BOOST_LOG_TRIVIAL(debug)
...
void Server::handleRequestScreenAccess(
    std::shared_ptr<UniversalConnection> /*conn*/,
    std::shared_ptr<matrixserver::MatrixServerMessage> /*msg*/) {
...
void Server::handleSetScreenFrame(
    std::shared_ptr<UniversalConnection> conn,
    std::shared_ptr<matrixserver::MatrixServerMessage> msg) {
    bool isTopApp = false;
    {
        ...
        registry_.forEachRenderer([this, msg, conn](std::shared_ptr<IRenderer> renderer) {
            for (const auto& screenInfo : msg->screendata()) {
                ...
                renderer->setScreenData(sid,
                                        (Color *)screenInfo.framedata().data());
            }
            ...
            if (msg->has_serverconfig()) {
                renderer->setGlobalBrightness(
                    msg->serverconfig().globalscreenbrightness());
            }
        });
    } else {
        // Send app to pause (not top app).
        BOOST_LOG_TRIVIAL(debug)
            << "[Server] send app " << msg->appid() << " to pause";
...
void Server::handleAppParamSchema(
    std::shared_ptr<UniversalConnection> /*conn*/,
    std::shared_ptr<matrixserver::MatrixServerMessage> msg) {
