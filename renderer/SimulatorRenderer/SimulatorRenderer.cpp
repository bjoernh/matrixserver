...
SimulatorRenderer::SimulatorRenderer(
    std::vector<std::shared_ptr<Screen>> screens, std::string setServerAddress,
    std::string setServerPort)
    : mainThread(), io_context(), serverAddress(setServerAddress),
...
bool SimulatorRenderer::connect() {
  BOOST_LOG_TRIVIAL(debug) << "[Renderer] Trying to connect to Simulator at "
  ...
  if (!connection->isDead()) {
    ...
    //        connection->setReceiveCallback(bind(&MatrixApplication::handleRequest,
...
int SimulatorRenderer::getGlobalBrightness() { return globalBrightness; }
