...
WebSocketSimulatorRenderer::WebSocketSimulatorRenderer(std::vector<std::shared_ptr<Screen>> initScreens, std::string setPort, bool setStreamPixels)
    ...
    BOOST_LOG_TRIVIAL(info) << "[WebSocketRenderer] Listening on WebSocket port " << port;
...
void WebSocketSimulatorRenderer::setClientMessageCallback(MsgCallback cb) { clientMessageCb = cb; }
...
void WebSocketSimulatorRenderer::do_accept() {
    ...
    acceptor->async_accept([this](beast::error_code ec, tcp::socket socket) {
        if (!ec) {
            ...
            auto session = std::make_shared<WsSession>(std::move(socket), clientMessageCb, [this]() {
            ...
            BOOST_LOG_TRIVIAL(debug) << "[WebSocketRenderer] Accept error: " << ec.message();
        }
        if (running)
            do_accept();
    });
...
void WebSocketSimulatorRenderer::render() {
    ...
    for (auto &screen : screens) {
        ...
        screenData->set_framedata(reinterpret_cast<const char *>(screen->getScreenData().data()), screen->getScreenDataSize() * sizeof(Color));
    ...
    if (!message.SerializeToString(&serialised)) {
        BOOST_LOG_TRIVIAL(debug) << "[WebSocketRenderer] Failed to serialise protobuf message";
        return;
    }
...
void WebSocketSimulatorRenderer::setGlobalBrightness(int brightness) { globalBrightness = brightness; }

int WebSocketSimulatorRenderer::getGlobalBrightness() { return globalBrightness; }
