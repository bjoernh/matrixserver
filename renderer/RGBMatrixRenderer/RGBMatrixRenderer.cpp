...
RGBMatrixRenderer::RGBMatrixRenderer(
    std::vector<std::shared_ptr<Screen>> initScreens) {
  init(initScreens);
}
...
void RGBMatrixRenderer::init(std::vector<std::shared_ptr<Screen>> initScreens) {
  ...
  RGBmatrixOptions.cols =
      64; // todo: should be tested -> tested now, no difference
...
void RGBMatrixRenderer::render() {
  ...
  for (auto screen : screens) {
    ...
    case Rotation::rot0:
      for (int i = 0, x = screen->getWidth() * screen->getOffsetX();
           i < screen->getWidth(); i++, x++) {
        for (int j = 0, y = screen->getHeight() * screen->getOffsetY();
             j < screen->getHeight(); j++, y++) {
          tempPixelColor = ScreenData[j + i * screen->getWidth()];
          rgbFrameCanvas->SetPixel(x, y, tempPixelColor.r(), tempPixelColor.g(),
                                   tempPixelColor.b());
        }
      }
      break;
  ...
  //    auto usTotal =
  //    std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::system_clock::now().time_since_epoch())
  //    - usStart; std::cout << usTotal.count() << " us" << std::endl; // ~ 15ms
  rgbFrameCanvas = rgbMatrix->SwapOnVSync(rgbFrameCanvas);
  renderMutex.unlock();
}
...
int RGBMatrixRenderer::getGlobalBrightness() { return globalBrightness; }
