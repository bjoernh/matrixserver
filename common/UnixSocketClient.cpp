...
std::shared_ptr<SocketConnection> UnixSocketClient::connect(boost::asio::io_context &io, std::string socketFile) {
    ...
    try {
        ...
        if (sockConnection->getSocket().is_open()) {
            BOOST_LOG_TRIVIAL(debug) << "[UnixSocketClient] Connect successful to path: " << unix_endpoint.path();
