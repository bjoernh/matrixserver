...
UnixSocketServer::UnixSocketServer(boost::asio::io_context &setIo, boost::asio::local::stream_protocol::endpoint setEndpoint)
    : io(setIo), endpoint(setEndpoint), acceptor(setIo) {
...
void UnixSocketServer::setAcceptCallback(std::function<void(std::shared_ptr<SocketConnection>)> callback) { acceptCallback = callback; }
