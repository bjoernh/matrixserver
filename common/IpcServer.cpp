    ...
    while(1){
        try {
            ...
            if (this->serverMQ->timed_receive(&receiveBuffer, SERVERMESSAGESIZE, recvd_size, priority, timeout)) {
                std::string clientMQName(receiveBuffer, recvd_size);
                ...
                auto sendMQ = std::make_shared<boost::interprocess::message_queue>(boost::interprocess::open_only, clientMQName.c_str());
                sendMQ->send(sendMQname.str().data(), sendMQname.str().size(), 0);

                BOOST_LOG_TRIVIAL(debug) << "[Server] Accepted Connection, clientMQ " << clientMQName << " receiveMQ " << sendMQname.str();

                auto connection = std::make_shared<IpcConnection>(sendMQ, receiveMQ, sendMQname.str());
                if (acceptCallback != NULL) {
                    acceptCallback(connection);
                    // Connection starts receiving via setReceiveCallback in the acceptCallback
                }
            }
            ...
            BOOST_LOG_TRIVIAL(error) << "[IpcServer] Unexpected error in acceptLoop: " << e.what();
            continue;
        } catch (...) {
            BOOST_LOG_TRIVIAL(error) << "[IpcServer] Unknown error in acceptLoop";
            continue;
        }
    }
}
