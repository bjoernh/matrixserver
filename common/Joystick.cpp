...
Joystick::Joystick(std::string devicePath, bool blocking) { init(devicePath, blocking); }
...
void Joystick::init(std::string devicePath, bool blocking) {
    ...
    if (devicePath.find("/dev/input/js") != std::string::npos) {
        try {
            joystickNumber_ = std::stoi(devicePath.substr(13));
        } catch (...) {
        }
    }
...
bool Joystick::isFound() {
    ...
    if (useSimulatorFallback_) {
        ...
        if (it == simulatorLastUpdate_.end())
            return false;
...
bool Joystick::getButton(unsigned int num) {
    if (useSimulatorFallback_) {
        ...
        if (num < MAXBUTTONAXISCOUNT)
            return simulatorStates_[joystickNumber_].button_[num];
        return false;
    }
    if (num < MAXBUTTONAXISCOUNT)
        return button_[num];
...
float Joystick::getAxis(unsigned int num) {
    if (useSimulatorFallback_) {
        std::lock_guard<std::mutex> lock(simulatorMutex_);
        if (num < MAXBUTTONAXISCOUNT)
            return simulatorStates_[joystickNumber_].axis_[num];
        return 0.0f;
    }
    if (num < MAXBUTTONAXISCOUNT)
        return axis_[num];
...
void Joystick::updateSimulatorState(const matrixserver::JoystickData& data) {
    ...
    float dpadx = 0.0f;
    if (data.buttondpadleft())
        dpadx = -1.0f;
    else if (data.buttondpadright())
        dpadx = 1.0f;

    float dpady = 0.0f;
    if (data.buttondpadup())
        dpady = -1.0f;
    else if (data.buttondpaddown())
        dpady = 1.0f;
...
std::ostream& operator<<(std::ostream& os, const Joystick::Event& e) {
    os << "type=" << static_cast<int>(e.type) << " number=" << static_cast<int>(e.number) << " value=" << static_cast<int>(e.value);
    return os;
}
