...
void MessageDispatcher::registerHandler(matrixserver::MessageType type, Handler h)
{
    handlers_[static_cast<int>(type)] = std::move(h);
}
    ...
    std::shared_ptr<matrixserver::MatrixServerMessage> msg)
{
    ...
    if (it == handlers_.end()) {
        BOOST_LOG_TRIVIAL(trace)
            << "[MessageDispatcher] No handler for message type "
            << msg->messagetype() << " — dropping";
        return;
    }
