...
CubeApplication::CubeApplication(int fps, std::string serverUri, std::string appName)
    :
#ifdef MATRIXAPPLICATION_STANDALONE
      MatrixApplicationStandalone(fps, serverUri, appName),
#else
      MatrixApplication(fps, serverUri, appName),
#endif
...
void CubeApplication::setPixel3D(int x, int y, int z, Color col,
                                 float intensity, bool add) {
  ...
  if (y == 0)
    screens[0]->setPixel(x - 1, z - 1, col,
                         add); // Screen 0: 3d_y = 0; x = 3d_x; y = 3d_z;
  else if (x == VIRTUALCUBEMAXINDEX)
    screens[1]->setPixel(y - 1, z - 1, col,
                         add); // Screen 1: 3d_x = 63; 3d_y = x; 3d_z = y;
  else if (y == VIRTUALCUBEMAXINDEX)
    screens[2]->setPixel(VIRTUALCUBEMAXINDEX - x - 1, z - 1, col,
                         add); // Screen 2: 3d_x = 63-x; 3d_y = 63; 3d_z = y;
  else if (x == 0)
    screens[3]->setPixel(VIRTUALCUBEMAXINDEX - y - 1, z - 1, col,
                         add); // Screen 3: 3d_x = 0; 3d_y = 63-x; 3d_z = y;
  else if (z == 0)
    screens[4]->setPixel(x - 1, VIRTUALCUBEMAXINDEX - y - 1, col,
                         add); // Screen 4: 3d_x = x; 3d_y = 63-y; 3d_z = 0;
  else if (z == VIRTUALCUBEMAXINDEX)
    screens[5]->setPixel(x - 1, y - 1, col,
                         add); // Screen 5: 3d_x = x; 3d_y = y; 3d_z = 63;
}
...
void CubeApplication::setPixelSmooth3D(float x, float y, float z, Color color) {
  ...
  if (rX != 0)
    setPixel3D(cX, fY, fZ, color, rX * (1.0f - rY) * (1.0f - rZ), true);
  if (rY != 0)
    setPixel3D(fX, cY, fZ, color, (1.0f - rX) * rY * (1.0f - rZ), true);
  if (rX != 0 && rY != 0)
    setPixel3D(cX, cY, fZ, color, rX * rY * (1.0f - rZ), true);
  if (rZ != 0)
    setPixel3D(fX, fY, cZ, color, (1.0f - rX) * (1.0f - rY) * rZ, true);
  if (rX != 0 && rZ != 0)
    setPixel3D(cX, fY, cZ, color, rX * (1.0f - rY) * rZ, true);
  if (rY != 0 && rZ != 0)
    setPixel3D(fX, cY, cZ, color, (1.0f - rX) * rY * rZ, true);
  if (rX != 0 && rY != 0 && rZ != 0)
    setPixel3D(cX, cY, cZ, color, rX * rY * rZ, true);
}
...
Color CubeApplication::getPixel3D(int x, int y, int z) {
  if (y == 0)
    return screens[0]->getPixel(
        x - 1, z - 1); // Screen 0: 3d_y = 0; x = 3d_x; y = 3d_z;
  else if (x == VIRTUALCUBEMAXINDEX)
    return screens[1]->getPixel(
        y - 1, z - 1); // Screen 1: 3d_x = 63; 3d_y = x; 3d_z = y;
  else if (y == VIRTUALCUBEMAXINDEX)
    return screens[2]->getPixel(
        VIRTUALCUBEMAXINDEX - x - 1,
        z - 1); // Screen 2: 3d_x = 63-x; 3d_y = 63; 3d_z = y;
  else if (x == 0)
    return screens[3]->getPixel(
        VIRTUALCUBEMAXINDEX - y - 1,
        z - 1); // Screen 3: 3d_x = 0; 3d_y = 63-x; 3d_z = y;
  else if (z == 0)
    return screens[4]->getPixel(
        x - 1, VIRTUALCUBEMAXINDEX - y -
                   1); // Screen 4: 3d_x = x; 3d_y = 63-y; 3d_z = 0;
  else if (z == VIRTUALCUBEMAXINDEX)
    return screens[5]->getPixel(
        x - 1, y - 1); // Screen 5: 3d_x = x; 3d_y = y; 3d_z = 63;

  BOOST_LOG_TRIVIAL(warning)
      << "Warning: getPixel3D called for coordinates (" << x << ", " << y
      << ", " << z
...
void CubeApplication::drawLine3D(int x1, int y1, int z1, const int x2,
                                 const int y2, const int z2, Color col) {
  drawLine3D(Vector3i(x1, y1, z1), Vector3i(x2, y2, z2), col);
}

void CubeApplication::drawLine2D(ScreenNumber screenNr, Vector2i start,
                                 Vector2i end, Color col) {
  drawLine2D(screenNr, start[0], start[1], end[0], end[1], col);
}

void CubeApplication::drawLine2D(ScreenNumber screenNr, int x0, int y0, int x1,
                                 int y1, Color col) {
...
void CubeApplication::drawText(ScreenNumber screenNr, Vector2i topLeftPoint,
                               Color col, std::string text) {
...
void CubeApplication::drawBitmap1bpp(ScreenNumber screenNr,
                                     Vector2i topLeftPoint, Color col,
                                     Bitmap1bpp bitmap) {
  for (auto pos : bitmap) {
...
// void CubeApplication::drawImage(ScreenNumber screenNr, Vector2i topLeftPoint,
// Magick::Image image){
//     for(uint cols = 0; cols < image.columns(); cols++){
//         for(uint rows = 0; rows < image.rows(); rows++){
//             Magick::Color col = image.pixelColor(cols, rows);
//             setPixel3D(getPointOnScreen(screenNr, Vector2i(cols, rows)),
//             Color(col.redQuantum(), col.greenQuantum(), col.blueQuantum()));
//         }
//     }
// }
...
// enum CornerNumber { frontRightTop, rightBackTop, backLeftTop, leftFrontTop,
// frontRightBottom, rightBackBottom, backLeftBottom, leftFrontBottom, anyCorner
// };
...
CornerNumber CubeApplication::getCornerNumber(Vector3i point) {
  ...
  if (point[0] == VIRTUALCUBEMAXINDEX && point[1] == 0 && point[2] == 0)
    ...
    result = leftFrontTop;
  else if (point[0] == VIRTUALCUBEMAXINDEX && point[1] == 0 &&
           point[2] == VIRTUALCUBEMAXINDEX)
...
Vector3i CubeApplication::getRandomPointOnScreen(ScreenNumber screenNr) {
  ...
  case front:
    result =
        Vector3i(rand() % VIRTUALCUBEMAXINDEX, 0, rand() % VIRTUALCUBEMAXINDEX);
    break;
  case right:
    result = Vector3i(VIRTUALCUBEMAXINDEX, rand() % VIRTUALCUBEMAXINDEX,
                      rand() % VIRTUALCUBEMAXINDEX);
    break;
  case back:
    result = Vector3i(rand() % VIRTUALCUBEMAXINDEX, VIRTUALCUBEMAXINDEX,
                      rand() % VIRTUALCUBEMAXINDEX);
    break;
  case left:
    result =
        Vector3i(0, rand() % VIRTUALCUBEMAXINDEX, rand() % VIRTUALCUBEMAXINDEX);
    break;
  case top:
    result =
        Vector3i(rand() % VIRTUALCUBEMAXINDEX, rand() % VIRTUALCUBEMAXINDEX, 0);
    break;
  case bottom:
    result = Vector3i(rand() % VIRTUALCUBEMAXINDEX,
                      rand() % VIRTUALCUBEMAXINDEX, VIRTUALCUBEMAXINDEX);
    break;
  }
  return result;
}

Vector3i CubeApplication::getPointOnScreen(ScreenNumber screenNr,
                                           Vector2i point) {
  Vector3i result;
  point += Vector2i(1, 1);
  switch (screenNr) {
  ...
  case back:
    result =
        Vector3i(VIRTUALCUBEMAXINDEX - point[0], VIRTUALCUBEMAXINDEX, point[1]);
    break;
  case left:
...
Vector3f CubeApplication::getPointOnScreen(ScreenNumber screenNr,
                                           Vector2f point) {
  Vector3f result;
  point += Vector2f(1, 1);
  switch (screenNr) {
  ...
  case back:
    result =
        Vector3f(VIRTUALCUBEMAXINDEX - point[0], VIRTUALCUBEMAXINDEX, point[1]);
    break;
  case left:
...
void CubeApplication::drawCircle2D(ScreenNumber screenNr, int x0, int y0,
...
void CubeApplication::drawRect2D(ScreenNumber screenNr, int x0, int y0, int x1,
                                 int y1, Color col, bool filled,
                                 Color fillColor) {
  if (filled) {
    for (int i = x0; i < x1; i++) {
...
void CubeApplication::drawRectCentered2D(ScreenNumber screenNr, int x0, int y0,
                                         int w, int h, Color col, bool filled,
                                         Color fillColor) {
  drawRect2D(screenNr, x0 - (w / 2), y0 - (h / 2), x0 + (w / 2), y0 + (w / 2),
             col, filled, fillColor);
}
