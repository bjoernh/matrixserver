...
void CubeApplication::setPixelSmooth3D(Vector3f pos, Color color) { return setPixelSmooth3D(pos[0], pos[1], pos[2], color); }
...
Color CubeApplication::getPixel3D(Vector3i pos) { return getPixel3D(pos[0], pos[1], pos[2]); }
...
Color CubeApplication::getPixel3D(int x, int y, int z) {
    if (y == 0)
        ...
        return screens[2]->getPixel(VIRTUALCUBEMAXINDEX - x - 1,
                                    z - 1); // Screen 2: 3d_x = 63-x; 3d_y = 63; 3d_z = y;
    else if (x == 0)
        return screens[3]->getPixel(VIRTUALCUBEMAXINDEX - y - 1,
...
bool CubeApplication::isActivePixel3D(Vector3i pos) { return isActivePixel3D(pos[0], pos[1], pos[2]); }

bool CubeApplication::isActivePixel3D(int x, int y, int z) { return getPixel3D(x, y, z) != Color::black(); }
