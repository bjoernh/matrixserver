...
void AnimationParams::registerFloat(const std::string& key, const std::string& label, float min, float max, float def, float step, const std::string& group) {
...
void AnimationParams::registerEnum(const std::string& key, const std::string& label, const std::vector<std::string>& options, const std::string& def, const std::string& group) {
    std::lock_guard<std::mutex> lock(mutex_);
    ParamRecord record;
    record.def.set_key(key);
    record.def.set_label(label);
...
float AnimationParams::getFloat(const std::string& key) const {
    ...
    if (it != params_.end()) return it->second.floatVal;
    return 0.0f;
}
...
int AnimationParams::getInt(const std::string& key) const {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = params_.find(key);
    if (it != params_.end()) return it->second.intVal;
    return 0;
}
...
bool AnimationParams::getBool(const std::string& key) const {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = params_.find(key);
    if (it != params_.end()) return it->second.boolVal;
    return false;
}
...
std::string AnimationParams::getString(const std::string& key) const {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = params_.find(key);
    if (it != params_.end()) return it->second.stringVal;
    return "";
}
