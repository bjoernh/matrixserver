...
void LoopRunner::start(BodyCallback body, PostBodyCallback postBody) {
  thread_ = std::make_unique<std::thread>(&LoopRunner::runLoop, this,
                                          std::move(body), std::move(postBody));
}
    ...
    try {
      ...
      if (state_ == AppState::running) {
        ...
        screenAccessCv_.wait(lock, [this] {
          return screenAccessGranted_ || !running_.load();
        });
        screenAccessGranted_ = false;
      }
...
void LoopRunner::setState(AppState state) {
  state_ = state;
}

AppState LoopRunner::getState() const {
  return state_;
}

void LoopRunner::setFps(int fps) {
  fps_ = fps;
}

int LoopRunner::getFps() const {
  return fps_;
}

float LoopRunner::getLoad() const {
  return load_;
}
